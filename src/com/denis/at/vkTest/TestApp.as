package com.denis.at.vkTest {
	...
	public class TestApp extends Sprite {
		...
		private function wallPostSuccess(data:Object):void {
			tf.appendText("Success wall.post post_id: " +
		...
		private function init(e:Event = null):void {
			...
			if (flashVars.api_id) {
				...
				tf.appendText("flashVars['api_id'] = " + flashVars['api_id'] +
						";\n");
				tf.appendText("flashVars['viewer_id'] = " +
						flashVars['viewer_id'] + ";\n");
			...
			VK.api('getProfiles', { uids: flashVars['viewer_id'] },
					fetchUserInfo, onApiRequestFail);
			...
			VK.addEventListener('onBalanceChanged',
					function(e:CustomEvent):void {
						tf.appendText("Balance changed: " + e.params[0] + "\n");
					});
			VK.addEventListener('onSettingsChanged',
					function(e:CustomEvent):void {
						tf.appendText("Settings changed: " + e.params[0] +
								"\n");
					});
			VK.addEventListener('onMerchantPaymentFail',
					function(e:CustomEvent):void {
						tf.appendText("Payment fail: " + e.params[0] + "\n");
					});
			VK.addEventListener('onMerchantPaymentSuccess',
					function(e:CustomEvent):void {
			...
			btn1.addEventListener(MouseEvent.CLICK, function(e:Event):void {
				VK.api('wall.post', { message: 'some text' }, wallPostSuccess,
						wallPostFail);
			});
			...
			btn4.addEventListener(MouseEvent.CLICK, function(e:Event):void {
				VK.callMethod("showMerchantPaymentBox", {
					...
					item_currency_1: 643,
					item_quantity_1: 1,
					item_photo_url_1: 'http://vkTest.com/images/gifts/96/149.png',
					item_digital_1: 1
				});
			});
